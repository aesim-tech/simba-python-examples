...
fig, axs = plt.subplots(nrows=len(modulations), ncols=len(rpm_speed_refs), layout='constrained')
fig.set_size_inches(16, 9)

for op_index, op_key in enumerate(['op_point1', 'op_point2']):
    for mod_item_index, mod_key in enumerate(modulations):
        ...
        for fpwm in switching_frequencies:
            df_mask_index = (res['speed']==rpm_speed_refs[op_key]) & (res['modulation']==mod_key)
            loss_plot = [res.loc[df_mask_index, ['conduction']],
                            res.loc[df_mask_index, ['switching']],
                            res.loc[df_mask_index, ['copper']],
                            res.loc[df_mask_index, ['iron']]]
...
# Add some text for labels, title and custom x-axis tick labels, etc.
    axs[mod_item_index, op_index].set_xlabel('Loss Types')
    axs[mod_item_index, op_index].set_ylabel('Losses (W)')
    axs[mod_item_index, op_index].set_title(f'Loss comparison at T = {torque_refs[op_key]} N.m & N = {rpm_speed_refs[op_key]} rpm for {mod_key}')
