...
num_bars = len(switches)
width = 0.5/(num_bars+1)  # the width of the bars
"""
...
fig, axs = plt.subplots(nrows=len(topos), ncols=1, layout='constrained')
...
for topo_index, topo_key in enumerate(topos):
    ...
    axs[topo_index].legend(loc='upper center', ncols=num_bars, fontsize=7)
    axs[topo_index].set_ylim(0, ax1_ymax*1.25)
    ax2.set_ylim(0, ax2_ymax*1.25)
